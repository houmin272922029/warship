...
DailyNewComposeChooseRequireViewOwner = DailyNewComposeChooseRequireViewOwner or {}
ccb["DailyNewComposeChooseRequireViewOwner"] = DailyNewComposeChooseRequireViewOwner


local function closeItemClick()
    popUpCloseAction(DailyNewComposeChooseRequireViewOwner, "infoBg", _layer )
end
DailyNewComposeChooseRequireViewOwner["closeItemClick"] = closeItemClick
...
local function confirmItemClick(tag)
    if _selected == -1 then
        ...
        print("------------把一件装备放入合成机器区域中")
        getDecomposeAndComposeLayer():newChooseRequireFinished(equipContent,_tag-1000)
        popUpCloseAction(DailyNewComposeChooseRequireViewOwner,"infoBg", _layer)
    end
end
DailyNewComposeChooseRequireViewOwner["confirmItemClick"] = confirmItemClick

local function cancelItemClick(tag) 
    popUpCloseAction(DailyNewComposeChooseRequireViewOwner, "infoBg", _layer )
end
DailyNewComposeChooseRequireViewOwner["cancelItemClick"] = cancelItemClick
...
local function _addTableView()
    -- 得到数据
    AllEquipsData = equipdata:getSameEquipById(_equipId,_equipUId)
    local equipsData1 = {}
...
local function onTouchBegan(x, y)
    ...
    local infoBg = tolua.cast(DailyNewComposeChooseRequireViewOwner["infoBg"], "CCSprite")
...
local function setMenuPriority()
    local menu = tolua.cast(DailyNewComposeChooseRequireViewOwner["menu"], "CCMenu")
    menu:setTouchPriority(_priority - 2)
   _tableView:setTouchPriority(_priority - 2)
end

function getRequireEquipChooseLayer()
    return _layer
end
...
local function _init()
    -- get layer
    local  proxy = CCBProxy:create()
    local  node  = CCBReaderLoad("ccbResources/DailyDrinkChoseHeroView.ccbi",proxy, true,"DailyNewComposeChooseRequireViewOwner")
    _layer = tolua.cast(node,"CCLayer")
    _contentLayer = tolua.cast(DailyNewComposeChooseRequireViewOwner["contentLayer"], "CCLayer")
    local title = tolua.cast(DailyNewComposeChooseRequireViewOwner["title"], "CCLabelTTF")
    title:setString( HLNSLocalizedString("daily.DC.pleaseChooseRequire"))
end

function createRequireEquipChooseLayer(priority,tag,equipContent) --priority -140
    _equipId = equipContent.equip.id
    _equipUId = equipContent.id
    _tag = tag
    ...
    local function _onEnter()
        print("NewRequireChooseLayer onEnter")
  
        _addTableView()     
        local seq = CCSequence:createWithTwoActions(CCDelayTime:create(0.5), CCCallFunc:create(setMenuPriority))
        _layer:runAction(seq)
        popUpUiAction(DailyNewComposeChooseRequireViewOwner, "infoBg")
        
    end

    local function _onExit()
        print("NewRequireChooseLayer onExit")
        _layer = nil
        _tableView = nil
        AllEquipsData = {}
        _selected = -1
