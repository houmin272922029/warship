...
local function getMyServerData()
    -- body
    local temp = {}
    if _data.serverList and _data.serverList ~= "" then
        for i=1,getMyTableCount(_data.serverList.server) do
            if _data.serverList.server[tostring(i)] and _data.serverList.server[tostring(i)] ~= "" then
                table.insert(temp,_data.serverList.server[tostring(i)])
            end
        end
    end
    return temp
        ...
        if fn == "cellSize" then
            ...
            local message = _rankArray[a1 +1]
                ...
                if helpHeightArray[a1+1] then
                    ...
                    cellHeight = getCellHeight(message, 620.0, 28,"ccbResources/FZCuYuan-M03S") + 20
        ...
        elseif fn == "cellAtIndex" then
            ...
            for i=1,2 do
                ...
                if message then
                    ...
                    local temp = CCLabelTTF:create(message, "ccbResources/FZCuYuan-M03S", 28)
...
-- 该方法名字每个文件不要重复
function getUnionRacingBattleServerListLayer()
    return _layer
end
...
function createUnionRacingBattleServerListLayer(data)
