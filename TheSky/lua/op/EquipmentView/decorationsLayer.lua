...
-- ·名字不要重复
DecorationViewOwner = DecorationViewOwner or {}
ccb["DecorationViewOwner"] = DecorationViewOwner

local function _addTableView()
    -- 得到数据
    -- shopContent = shopData:getItemShopData()
    local _topLayer = DecorationViewOwner["titleLayer"]
    beltArray = equipdata:getAllBeltData()
    ...
    local function onEquipSell4Click()
        print("decoration click")
        if getMainLayer() ~= nil then
            getMainLayer():gotoEquipSellLayer(EquipSellType.Decoration)
        end
    end

    EquipSellCellOwner["onEquipSellClick"] = onEquipSell4Click
   ...
   local function refineCallBack( url,rtnData )
    ...
    local function refineBtnTaped( tag,sender )
        local uniqueId = beltArray[tag]["id"]

        if getMainLayer() ~= nil then
            runtimeCache.decorationTableOffsetY = _tableView:getContentOffset().y
            getMainLayer():getoEquipRefineLayer(uniqueId)
        end
        
        -- doActionFun("REFINE_EQUIP_URL",{content.id},refineCallBack)
    end

    local function updateBtnTaped( tag,sender )
        local content = beltArray[tag]
        if getMainLayer() ~= nil then
            runtimeCache.decorationTableOffsetY = _tableView:getContentOffset().y
    ...
    local h = LuaEventHandler:create(function(fn, table, a1, a2)
        local r
        if fn == "cellSize" then
            if a1 == getMyTableCount(beltArray) + 1 then
        ...
        elseif fn == "cellAtIndex" then
            ...
            --单条数据
            local equipContent = beltArray[a1]
            ...
            elseif a1 == 0 then
                ...
                local runeSprite = tolua.cast(EquipmentsTableCellOwner["runeSprite"],"CCSprite")
                runeSprite:setVisible(false)
                if equipContent.equip.rune then
                    runeSprite:setVisible(true)
                    runeSprite:setDisplayFrame(CCSpriteFrameCache:sharedSpriteFrameCache():spriteFrameByName(string.format("nature_%d.png", equipContent.equip.rune)))
                end
...
local function _init()
    -- get layer
    local  proxy = CCBProxy:create()
    local  node  = CCBReaderLoad("ccbResources/DecorationView.ccbi",proxy, true,"DecorationViewOwner")
    _layer = tolua.cast(node,"CCLayer")
end


-- 该方法名字每个文件不要重复
function getDecorationsLayer()
	return _layer
end

function createDecorationsLayer()
    _init()
    ...
    local function _onEnter()
        cellArray = {}
        _addTableView()
        if runtimeCache.decorationTableOffsetY then
            _tableView:setContentOffset(ccp(0, runtimeCache.decorationTableOffsetY))
            runtimeCache.decorationTableOffsetY = nil
        end
        generateCellAction( cellArray,getMyTableCount(beltArray) + 2 )
        cellArray = {}
    end
