...
-- ·名字不要重复
AlloEquipsViewOwner = AlloEquipsViewOwner or {}
ccb["AlloEquipsViewOwner"] = AlloEquipsViewOwner

local function _addTableView()
    -- 得到数据
    local _topLayer = AlloEquipsViewOwner["titleLayer"]
    ...
    local function refineBtnTaped( tag,sender )
        local uniqueId = AllEquipsData[tag]["id"]

        if getMainLayer() ~= nil then
            runtimeCache.allEquipsTableOffsetY = _tableView:getContentOffset().y
            getMainLayer():getoEquipRefineLayer(uniqueId)
        end
    end

    local function updateBtnTaped( tag,sender )
    Global:instance():TDGAonEventAndEventData("intensify")
        local content = AllEquipsData[tag]
        if getMainLayer() ~= nil then
            runtimeCache.allEquipsTableOffsetY = _tableView:getContentOffset().y
    ...
    local function onAvatarBtnTap( tag,sender )
        print("onAvatarBtnTap")
        local content = AllEquipsData[tag]
        if content then
            local equipLayer = createEquipInfoLayer(content["id"], 0, -135)
            getMainLayer():addChild(equipLayer, 2000)
        end
    end
    ...
    local function onEquipSellClick()
        Global:instance():TDGAonEventAndEventData("equipment4")
        print("onEquipSellClick")
        -- runtimeCache.allEquipsTableOffsetY = _tableView:getContentOffset().y
        if getMainLayer() ~= nil then
            getMainLayer():gotoEquipSellLayer(EquipSellType.AllEquip)
        end
    end

    EquipSellCellOwner["onEquipSellClick"] = onEquipSellClick
    ...
    local h = LuaEventHandler:create(function(fn, table, a1, a2)
        local r
        if fn == "cellSize" then
            if a1 == getMyTableCount(AllEquipsData) + 1 then
        ...
        elseif fn == "cellAtIndex" then
            ...
            elseif a1 == 0 then
                ...
                local equipContent = AllEquipsData[a1]
        ...
        elseif fn == "numberOfCells" then
            ...
            -- r = 5
            r = getMyTableCount(AllEquipsData) + 2
        -- Cell events:
...
local function _init()
    -- get layer
    local  proxy = CCBProxy:create()
    local  node  = CCBReaderLoad("ccbResources/AllEquipsView.ccbi",proxy, true,"AlloEquipsViewOwner")
    _layer = tolua.cast(node,"CCLayer")
end


-- 该方法名字每个文件不要重复
function getAllEquipsLayer()
	return _layer
end

function createAllEquipsLayer()
    _init()
    ...
    local function _onEnter()
        cellArray = {}
        _addTableView()
        if runtimeCache.allEquipsTableOffsetY then
            _tableView:setContentOffset(ccp(0, runtimeCache.allEquipsTableOffsetY))
            runtimeCache.allEquipsTableOffsetY = nil
        end
        generateCellAction( cellArray,getMyTableCount(AllEquipsData) + 2 )
        cellArray = {}
    end
