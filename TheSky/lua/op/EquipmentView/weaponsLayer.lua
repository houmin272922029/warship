...
-- ·名字不要重复
WeaponsViewOwner = WeaponsViewOwner or {}
ccb["WeaponsViewOwner"] = WeaponsViewOwner


local function _addTableView()
    -- 得到数据
    -- shopContent = shopData:getItemShopData()
    local _topLayer = WeaponsViewOwner["titleLayer"]
    WeaponsArray = equipdata:getAllWeaponsData()
    ...
    local function refineBtnTaped( tag,sender )
        local uniqueId = WeaponsArray[tag]["id"]

        if getMainLayer() ~= nil then
            runtimeCache.weaponsTableOffsetY = _tableView:getContentOffset().y
            getMainLayer():getoEquipRefineLayer(uniqueId)
        end
    end

    local function updateBtnTaped( tag,sender )
        local content = WeaponsArray[tag]
        if getMainLayer() ~= nil then
            runtimeCache.weaponsTableOffsetY = _tableView:getContentOffset().y
    ...
    local function onEquipSell2Click()
        print("onEquipSellClick")
        if getMainLayer() ~= nil then
            getMainLayer():gotoEquipSellLayer(EquipSellType.Weapons)
        end
    end

    EquipSellCellOwner["onEquipSellClick"] = onEquipSell2Click
    ...
    local h = LuaEventHandler:create(function(fn, table, a1, a2)
        local r
        if fn == "cellSize" then
            if a1 == getMyTableCount(WeaponsArray) + 1 then
        ...
        elseif fn == "cellAtIndex" then
            ...
            --单条数据
            local equipContent = WeaponsArray[a1]
        ...
        elseif fn == "numberOfCells" then
            ...
            -- r = 5
            r = getMyTableCount(WeaponsArray) + 2
        -- Cell events:
...
local function _init()
    -- get layer
    local  proxy = CCBProxy:create()
    local  node  = CCBReaderLoad("ccbResources/WeaponsView.ccbi",proxy, true,"WeaponsViewOwner")
    _layer = tolua.cast(node,"CCLayer")
end


-- 该方法名字每个文件不要重复
function getWeaponsLayer()
	return _layer
end

function createWeaponsLayer()
    _init()
    ...
    local function _onEnter()
        cellArray = {}
        _addTableView()
        if runtimeCache.weaponsTableOffsetY then
            _tableView:setContentOffset(ccp(0, runtimeCache.weaponsTableOffsetY))
            runtimeCache.weaponsTableOffsetY = nil
        end
        generateCellAction( cellArray,getMyTableCount(WeaponsArray) + 2 )
        cellArray = {}
    end
