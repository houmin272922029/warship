...
-- 名字不要重复
ExpenseRebateOwner = ExpenseRebateOwner or {}
ccb["ExpenseRebateOwner"] = ExpenseRebateOwner

local function closeItemClick()
    -- _layer:removeFromParentAndCleanup(true)

    popUpCloseAction( ExpenseRebateOwner,"infoBg",_layer )
end
ExpenseRebateOwner["closeItemClick"] = closeItemClick
...
ExpenseRebateOwner["payItemClick"] = payItemClick

local function helpItemClick()
    local desp = loginActivityData.activitys[Activity_Rebate2].description
    local array = {}
...
ExpenseRebateOwner["helpItemClick"] = helpItemClick

local function refreshTimeLabel()
    local timerLabel = tolua.cast(ExpenseRebateOwner["timerLabel"], "CCLabelTTF")
    local timer = loginActivityData:getExpenseRebateTimer()
    if timer == 0 then
        closeItemClick()
        getLoginActivityLayer():closeView()
    end
...
local function costRewardCallback(url, rtnData)
    loginActivityData.activitys[Activity_Rebate2] = rtnData.info.costReward
     _rebateData = loginActivityData:getExpenseRebateData()
...
local function _addTableView()
    ...
    local content = ExpenseRebateOwner["content"]
    ...
    local function rewardItemClick(tag)
        ...
        doActionFun("COST_REWARD", {dic.key}, costRewardCallback)
    end
    ...
    local h = LuaEventHandler:create(function(fn, table, a1, a2)
        ...
        elseif fn == "cellAtIndex" then
            ...
            tips2:setString(HLNSLocalizedString("expenseRebate.tips.2", dic.goldLimit))
            ...
            if tonumber(dic.status) == 1 then
                ...
                local gold = tonumber(loginActivityData.activitys[Activity_Rebate2].goldCost)
                if gold >= tonumber(dic.goldLimit) then
                    ...
                    tips1:setString(HLNSLocalizedString("expenseRebate.tips.1", dic.goldLimit - gold))
                end
            end
...
local function _refreshData()
    ...
    local expenseLabel = tolua.cast(ExpenseRebateOwner["expenseLabel"], "CCLabelTTF")
    expenseLabel:setString(loginActivityData.activitys[Activity_Rebate2].goldCost)

    local title0 = tolua.cast(ExpenseRebateOwner["title0"], "CCLabelTTF")
    local title1 = tolua.cast(ExpenseRebateOwner["title1"], "CCLabelTTF")
    title0:setString(loginActivityData.activitys[Activity_Rebate2].name)
    title1:setString(loginActivityData.activitys[Activity_Rebate2].name)
end
...
local function _init()
    -- get layer
    local  proxy = CCBProxy:create()
    local  node  = CCBReaderLoad("ccbResources/ExpenseRebateView.ccbi", proxy, true,"ExpenseRebateOwner")
...
local function onTouchBegan(x, y)
    ...
    local infoBg = tolua.cast(ExpenseRebateOwner["infoBg"], "CCSprite")
...
local function setMenuPriority()
    ...
    local menu = tolua.cast(ExpenseRebateOwner["menu"], "CCMenu")
    menu:setTouchPriority(_priority - 2)

    _tableView:setTouchPriority(_priority - 2)
end

function getExpenseRebateLayer()
	return _layer
end

function createExpenseRebateLayer(priority)
    ...
    local function _onEnter()
        ...
        popUpUiAction( ExpenseRebateOwner,"infoBg" )

        -- 设置活动时间倒计时
        addObserver(NOTI_EXPENSEREBATE, refreshTimeLabel)
    end
    ...
    local function _onExit()
        ...
        _priority = nil
        _rebateData = nil
        removeObserver(NOTI_EXPENSEREBATE, refreshTimeLabel)
    end
