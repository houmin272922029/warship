...
function CCBReaderLoad(strFilePath,proxy,bSetOwner,strOwnerName)
    ...
    local allNodes  = tolua.cast(ccbReader:getAllNodes(),"CCArray")
    for i = 1 , allNodes:count() do
        local node = tolua.cast(allNodes:objectAtIndex(i - 1),"CCNode")
        if proxy:getNodeTypeName(node) == "CCLabelTTF" then
            local label = tolua.cast(node, "CCLabelTTF")
            label:setString(HLNSLocalizedString(label:getString()))
        end 
    end 
