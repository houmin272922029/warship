local ItemView = qy.class("ItemView", qy.tank.view.BaseView, "recharge_doyen.ui.ItemView")
...
function ItemView:ctor(delegate)
    ...
    self.Image_1:setSwallowTouches(false)
   	self:OnClick("Button_1", function()
        ...
        elseif self.status == 2 then
            service:getCommonGiftAward(nil, qy.tank.view.type.ModuleType.RECHARGE_DOYEN,false, function(data)
                self.status = 5
                self:update()
            end)
    ...
    local rank = ccui.TextAtlas:create("2", "recharge_doyen/fx/level_number.png", 26, 25, '0')
...
function ItemView:setData(data, idx)
    ...
    if model.rechargeDoyenRankList[tostring(idx + 1)] then
        self.Name:setString(model.rechargeDoyenRankList[tostring(idx + 1)].nickname)
    else
        self.Name:setString(qy.TextUtil:substitute(60005))
    end

    -- 1去充值,2领取,3非本人不可领取,4充值金额不足不可领取，5已领取
    local rankData = model.rechargeDoyenRankList[tostring(idx + 1)]


    if qy.tank.model.UserInfoModel.serverTime >= model.rechargeDoyenEndTime then

            if rankData and rankData.kid == qy.tank.model.UserInfoModel.userInfoEntity.kid then
                if model.rechargeDoyenAmount >= data.amount then
                    if model.rechargeDoyendDrawAward == 0 then
