local MainDialog = qy.class("MainDialog", qy.tank.view.BaseDialog, "recharge_doyen.ui.Maindialog")
...
function MainDialog:createView()
    ...
    local function numberOfCellsInTableView(tableView)
        return #model.rechargeDoyenList
    end
    ...
    local function tableCellAtIndex(tableView, idx)
        ...
        if nil == cell then
            ...
            item = require("recharge_doyen.src.ItemView").new(self)
            cell:addChild(item)
            cell.item = item
        end

        cell.item:setData(model.rechargeDoyenList[idx + 1], idx)
        return cell
    end
