...
local MainDialog = qy.class("MainDialog", qy.tank.view.BaseDialog, "merge_carnival/ui/MainDialog")
...
function MainDialog:ctor(delegate)
    ...
    self:OnClick("day6",function ( sender )
        self:choosedayBt(6)
    end)
    self:OnClick("day7",function ( sender )
        self:choosedayBt(7)
    end)
    ...
    self:OnClick("Bt2",function ( sender )
    	self:updateBt(2)
    	self:updateBttask(self.chooseday,2)
    	-- body
    end)
    self:OnClick("Bt3",function ( sender )
    	self:updateBt(3)
    	 self:updateBttask(self.chooseday,3)
    	-- body
    end)
     self:OnClick("Bt4",function ( sender )
    	self:updateBt(4)
    	self:updateBttask(self.chooseday,4)
    	-- body
    end)
	...
	self:updateBt(1)
	self.day = self.model.day > 7 and 7 or self.model.day--总天数
	self.chooseday = self.day--选择的天
	self:updateDayBt(self.chooseday)
    self:updateBttask(self.chooseday,1)
    for i=1,self.day do
...
function MainDialog:updateDayBt(day)
	for i=1,7 do
		if i == day then
			self["day"..i]:loadTexture("merge_carnival/res/anniu2.png",1)
		else
			self["day"..i]:loadTexture("merge_carnival/res/anniu1.png",1)
		end
	end
end
...
function MainDialog:__createList(_idx)
	...
	local function tableCellAtIndex(table, idx)
		...
		if nil == cell then
			...
			item = require("merge_carnival.src.Cell").new({
				["value"] = _idx,
				["data"]= data2,
				["callback"] = function ()
