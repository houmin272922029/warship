local RecordDialog = qy.class("RecordDialog", qy.tank.view.BaseView, "inter_service_escort.ui.RecordDialog")
...
function RecordDialog:ctor(delegate, controller)
    ...
    local style = qy.tank.view.style.ViewStyle1.new({
        titleUrl = "inter_service_escort/res/yayunjilu1.png", 
        showHome = false,
        ["onExit"] = function()
...
function RecordDialog:createView()
    ...
    local function tableCellAtIndex(tableView, idx)
        ...
        if nil == cell then
            ...
            item = require("inter_service_escort.src.RecordItemView").new()
            cell:addChild(item)
            cell.item = item
        end
