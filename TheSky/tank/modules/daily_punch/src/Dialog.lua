...
local Dialog = qy.class("Dialog", qy.tank.view.BaseDialog, "daily_punch.ui.Layer")
...
function Dialog:__createList()
    ...
    local function tableCellAtIndex(tableView, idx)
        ...
        if nil == cell then
            ...
            item = require("daily_punch.src.Cell").new(self)
            cell:addChild(item)
            cell.item = item
        end
...
function Dialog:update()
    ...
    for i = 1, #self.model.score_award do
        if self.model.score_award[i].cny > self.model.punch then
            next_ = self.model.score_award[i].cny - self.model.punch 
            break
        end
    end
    ...
    if self.model.punch > self.model.score_award[#self.model.score_award].cny then
        self.model.punch = self.model.score_award[#self.model.score_award].cny
    end

    self.Text_num1:setString(tostring(self.model.punch))
