...
local RewardPreviewDialog = qy.class("RewardPreviewDialog", qy.tank.view.BaseDialog, "combat_casting.ui.RewardPreviewDialog")
...
function RewardPreviewDialog:ctor(delegate)
    ...
    self.style = qy.tank.view.style.DialogStyle1.new({
        ...
        position = cc.p(0,0),
        offset = cc.p(0,0),
        titleUrl = "combat_casting/res/20.png",
        bgShow = false,
...
function RewardPreviewDialog:__createList()
    ...
    local function tableCellAtIndex(tableView, idx)
        ...
        if nil == cell then
            ...
            item = require("combat_casting.src.RewardPreviewCell").new(self)
            cell:addChild(item)
            cell.item = item
        end

        cell.item:render(self.model.rank[idx+1], idx+1)
