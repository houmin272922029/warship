...
local WorldBoss = qy.class("Tank", qy.tank.view.BaseView)

function WorldBoss:ctor(entity,index)
    WorldBoss.super.ctor(self)
...
function WorldBoss:onCleanup()
end

-- 创建
function WorldBoss:create(entity)
	-- self.entity = entity 
...
-- 创建坦克机身
function WorldBoss:__createBody()
	qy.tank.utils.cache.CachePoolUtil.addPlist("tank/avatar/t59",1)
	self.body = qy.tank.widget.FrameAnimation.new("t59_body",6,0.2,0,function()
		self.body:getParent():removeChild(self.barrel)
		self.body = nil
	end) 
...
-- 创建炮管
function WorldBoss:__createBarrel()
	if not tolua.cast(self.barrel,"cc.Node") then
		...
		self.barrel = cc.Sprite:createWithSpriteFrameName("t59_barrel_1.png")
		self.middle:addChild(self.barrel)
	end 
end

-- 播放炮管动画
function WorldBoss:__playBarrel()
...
-- 停止炮管动画
function WorldBoss:removeBarrel()
	if tolua.cast(self.barrel,"cc.Node") then
		self.middle:removeChild(self.barrel)
		self.barrel = nil
	end
...
-- ================================================== Action Start ==================================================
-- 启动坦克
function WorldBoss:play()
	self:playRandomMove()
end

-- 停止坦克 
function WorldBoss:stop()
...
-- 射击
function WorldBoss:shoot(skillID,loop)
	print("++++++++++++++++++++++++++++++ WorldBoss:shoot")
...
-- 中弹
function WorldBoss:hurt(skillID)
	print("++++++++++++++++++++++++++++++ WorldBoss:hurt",skillID,qy.Config.skill[tostring(skillID)])
...
-- 将目标添加到地面层
function WorldBoss:addToGround(target)
...
-- 位移 1前 2后 3左 4右
function WorldBoss:move(direction,duration,distance,callback)
...
-- 开始随机运动
function WorldBoss:playRandomMove()
...
-- 停止随机运动
function WorldBoss:stopRandomMove()
	qy.Timer.remove("tank"..self.entity.direction.."_"..self.index)
	self:resetPosition()
end

-- 坐标复位
function WorldBoss:resetPosition()
...
-- 后坐力
function WorldBoss:__recoil(_offset)
	local offset = nil
	if _offset == nil then
		offset = 100
	else
...
function WorldBoss:__stopRecoil()
	if self.act then
...
-- 发射后的弹壳
function WorldBoss:__playShell()
...
-- 履带碾压轨迹
function WorldBoss:__showTrack()
...
function WorldBoss:hideTrack()
	qy.Timer.remove("tank_track"..self.entity.direction.."_"..self.index)
end
-- ================================================== Action Effect End ==================================================
-- ================================================== Skill Effect Start ==================================================
function WorldBoss:playFx(container,name,pos,rotation,scale,flipX,idx)
	print("++++++++++++++++++++++++++++++ WorldBoss:playFx",name)
...
-- ================================================== Skill Effect End ==================================================
-- ==================================================  Buff&Tip Start ==================================================
function WorldBoss:update(data,_type)
...
function WorldBoss:initUpdate(data)
...
function WorldBoss:fightUpdate(data)
	self.entity.blood = data.blood
	self.entity.morale = data.morale
...
function WorldBoss:skipUpdate(data)
	self.entity.blood = data.blood
	self.entity.morale = data.morale
	self.entity.hurt = data.hurt
	self.entity.add = data.add
...
-- 更新血量条状态
function WorldBoss:updateHP(smooth)
...
-- 改变血量&血量条&显示tip
function WorldBoss:changeHP(value)
...
function WorldBoss:showHPTip(value,delayShow)
	self:__createNumTip(value,20,delayShow)
end

function WorldBoss:updateMorale()
...
function WorldBoss:removeMorale()
	if tolua.cast(self.moraleLabel,"cc.Node") then
...
function WorldBoss:__createNumTip(value,h,delayShow)
	local valueStr = nil
...
function WorldBoss:popupTip(target,h,delay,type)
	if target then
...
function WorldBoss:popupTipSequence()
...
-- 状态复原
function WorldBoss:recover(data)
...
-- 反作用效果。作用于自身或己方
function WorldBoss:feedback(data)
	if data then
...
-- 主动加血技能
function WorldBoss:addHpBuffTo()
	self:playBuff(self.top,self.entity:getDefPos(),"buff_fx_jiaxueqs",1,nil,nil)
end

-- 被动加血技能
function WorldBoss:addHpBuffFrom(change,value)
...
-- 添加护盾技能
function WorldBoss:addDefenseBuff()
	local x = self.entity:getDefPos().x
	local y = self.entity:getDefPos().y - 20
...
-- 移除护盾技能
function WorldBoss:removeDefenseBuff()
...
function WorldBoss:playSkillBuffByID(id)
...
function WorldBoss:playBuff(container,pos,name,idx,duration,loop)
	print("++++++++++++++++++++++++++++++ WorldBoss:playBuff",name,idx,duration,loop)
...
function WorldBoss:setIndex(idx)
	self.index = idx
	-- self.indexLabel:setString(tostring(idx))
end

function WorldBoss:runCustomizeAction(actions)
end

function WorldBoss:getOrigin()
