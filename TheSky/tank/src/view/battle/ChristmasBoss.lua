...
local ChristmasBoss = qy.class("Tank", qy.tank.view.BaseView)

function ChristmasBoss:ctor(entity,index)
    ChristmasBoss.super.ctor(self)
...
function ChristmasBoss:onEnter()
	self:setPosition(self:getOrigin())
	self:create(self.entity)
end

function ChristmasBoss:onExit()
end

function ChristmasBoss:onCleanup()
end

-- 创建
function ChristmasBoss:create(entity)
	-- self.entity = entity 
	
	-- 坦克素材相关配置
	local pos = entity:getBuffPos()
	-- 血量标签
	self.bloodLabel = cc.Label:createWithSystemFont(qy.TextUtil:substitute(5009),"Helvetica", 12.0,cc.size(50,30))
    ...
    -- 士气标签
	self.moraleLabel = cc.Label:createWithSystemFont(qy.TextUtil:substitute(5010),"Helvetica", 12.0,cc.size(50,30))
...
-- 销毁
function ChristmasBoss:destroy()
...
-- 创建坦克机身
function ChristmasBoss:__createBody()
		cc.SpriteFrameCache:getInstance():addSpriteFrames("tank/avatar/t156.plist")
	self.tank = cc.Sprite:createWithSpriteFrameName("t156.png")
	self.middle:addChild(self.tank)
	self.tank:setScale(1)
end

-- 创建炮管
function ChristmasBoss:__createBarrel()
	if not tolua.cast(self.barrel,"cc.Node") then
		if cc.SpriteFrameCache:getInstance():getSpriteFrame(self.entity:getBarrelRes()) == nil then
			qy.tank.utils.cache.CachePoolUtil.addPlist("tank/avatar/t156",1)
		end
		self.barrel = cc.Sprite:createWithSpriteFrameName("156_1.png")
		self.middle:addChild(self.barrel)
	end 
end

-- 播放炮管动画
function ChristmasBoss:__playBarrel()
	if tolua.cast(self.barrel,"cc.Node") then
		self.middle:removeChild(self.barrel)
		self.barrel = nil
	end
	self.barrel = qy.tank.widget.FrameAnimation.new(self.entity:getResPrefix(),5,0.5,1,function()
...
-- 停止炮管动画
function ChristmasBoss:removeBarrel()
	if tolua.cast(self.barrel,"cc.Node") then
		self.middle:removeChild(self.barrel)
		self.barrel = nil
	end
...
-- ================================================== Action Start ==================================================
-- 启动坦克
function ChristmasBoss:play()
	self:playRandomMove()
end

-- 停止坦克 
function ChristmasBoss:stop()
...
-- 射击
function ChristmasBoss:shoot(skillID,loop)
	print("++++++++++++++++++++++++++++++ ChristmasBoss:shoot")
...
-- 中弹
function ChristmasBoss:hurt(skillID)
	print("++++++++++++++++++++++++++++++ ChristmasBoss:hurt",skillID,qy.Config.skill[tostring(skillID)])
...
-- 将目标添加到地面层
function ChristmasBoss:addToGround(target)
...
-- 位移 1前 2后 3左 4右
function ChristmasBoss:move(direction,duration,distance,callback)
...
-- 开始随机运动
function ChristmasBoss:playRandomMove()
...
-- 停止随机运动
function ChristmasBoss:stopRandomMove()
	qy.Timer.remove("tank"..self.entity.direction.."_"..self.index)
	self:resetPosition()
end

-- 坐标复位
function ChristmasBoss:resetPosition()
...
-- ================================================== Action Effect Start ==================================================
-- 播放发射火花
function ChristmasBoss:__playFire()
...
-- 后坐力
function ChristmasBoss:__recoil(_offset)
	local offset = nil
	if _offset == nil then
		offset = 100
	else
...
function ChristmasBoss:__stopRecoil()
	if self.act then
...
-- 发射后的弹壳
function ChristmasBoss:__playShell()
...
-- 履带碾压轨迹
function ChristmasBoss:__showTrack()
...
function ChristmasBoss:hideTrack()
	qy.Timer.remove("tank_track"..self.entity.direction.."_"..self.index)
end
-- ================================================== Action Effect End ==================================================
-- ================================================== Skill Effect Start ==================================================
function ChristmasBoss:playFx(container,name,pos,rotation,scale,flipX,idx)
	print("++++++++++++++++++++++++++++++ ChristmasBoss:playFx",name)
...
-- ================================================== Skill Effect End ==================================================
-- ==================================================  Buff&Tip Start ==================================================
function ChristmasBoss:update(data,_type)
...
function ChristmasBoss:initUpdate(data)
...
function ChristmasBoss:fightUpdate(data)
	self.entity.blood = data.blood
	self.entity.morale = data.morale
...
function ChristmasBoss:skipUpdate(data)
	self.entity.blood = data.blood
	self.entity.morale = data.morale
	self.entity.hurt = data.hurt
	self.entity.add = data.add
...
-- 更新血量条状态
function ChristmasBoss:updateHP(smooth)
	if self.entity and self.entity.blood then
		...
		if smooth then
	    	self.bloodBar:setPercent(tonumber(self.entity.blood)/tonumber(self.entity.max_blood),true,true)
		else
	    	self.bloodBar:setPercent(tonumber(self.entity.blood)/tonumber(self.entity.max_blood),true,false)
		end
	end
end

-- 移除血量条
function ChristmasBoss:removeHP()
	if tolua.cast(self.bloodLabel,"cc.Node") then
...
-- 改变血量&血量条&显示tip
function ChristmasBoss:changeHP(value)
...
function ChristmasBoss:showHPTip(value,delayShow)
	self:__createNumTip(value,20,delayShow)
end

function ChristmasBoss:updateMorale()
...
function ChristmasBoss:removeMorale()
	if tolua.cast(self.moraleLabel,"cc.Node") then
...
-- 取消缴械
function ChristmasBoss:unforbid()
...
function ChristmasBoss:__createNumTip(value,h,delayShow)
	local valueStr = nil
...
function ChristmasBoss:popupTip(target,h,delay,type)
	if target then
...
function ChristmasBoss:popupTipSequence()
...
-- 状态复原
function ChristmasBoss:recover(data)
...
-- 反作用效果。作用于自身或己方
function ChristmasBoss:feedback(data)
	if data then
...
-- 主动加血技能
function ChristmasBoss:addHpBuffTo()
	self:playBuff(self.top,self.entity:getDefPos(),"buff_fx_jiaxueqs",1,nil,nil)
end

-- 被动加血技能
function ChristmasBoss:addHpBuffFrom(change,value)
...
-- 添加护盾技能
function ChristmasBoss:addDefenseBuff()
	local x = self.entity:getDefPos().x
	local y = self.entity:getDefPos().y - 20
...
-- 移除护盾技能
function ChristmasBoss:removeDefenseBuff()
...
function ChristmasBoss:playSkillBuffByID(id)
...
function ChristmasBoss:playBuff(container,pos,name,idx,duration,loop)
	print("++++++++++++++++++++++++++++++ ChristmasBoss:playBuff",name,idx,duration,loop)
...
function ChristmasBoss:setIndex(idx)
	self.index = idx
	-- self.indexLabel:setString(tostring(idx))
end

function ChristmasBoss:runCustomizeAction(actions)
end

function ChristmasBoss:getOrigin()
	return cc.p(self.origin.x+self.offset.x,self.origin.y+self.offset.y)
end

-- ==================================================  Buff End ==================================================

return ChristmasBoss
