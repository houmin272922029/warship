local BaseAlert = qy.class("BaseAlert", qy.tank.widget.PopupWindowWrapper, "view/alert/BaseAlert")

function BaseAlert:ctor(delegate)
    BaseAlert.super.ctor(self)
...
function BaseAlert:showWithNode()
    self:setTitle()
...
function BaseAlert:addWithNode()
...
function BaseAlert:setBackgroundWithColor()

end

--设置title的内容和坐标
function BaseAlert:setTitle()
...
--content内容，富媒体文本
function BaseAlert:createRichText()
...
function BaseAlert:createBtns()
    ...
    for k,btn in pairs(btnArr) do
        ...
        self:OnClick(btn.Button_1, function()
...
--btn按钮样式工厂
function BaseAlert:getBtnStyleById(id)
    local btn
    if(id == 1) then
