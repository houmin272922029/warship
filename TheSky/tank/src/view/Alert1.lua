...
local Alert1 = qy.class("Alert1", qy.tank.view.BaseView)

function Alert1:ctor()
    Alert1.super.ctor(self)
end
...
function Alert1:show( title , content ,size , flags , btnClickFunc , extendObj)
    assert(content,"content不能为空")
    assert(title , "title不能为空")
    assert(size , "size不能为空")
    --print("---------!!!!!!!!!---->"..size.width)
    self.extendObj = extendObj
    self.baseAlert = qy.tank.view.BaseAlert3.new(
        {
            ["onClose"] = function()
...
function Alert1:showWithNode( title , contentNode , size , flags , btnClickFunc , extendObj)
...
--显示tip
--]]
function Alert1:showTip(contentNode)
    ...
    self:OnClick1(self.layout, function()
