local MainView = qy.class("MainView", qy.tank.view.BaseView, "view/main/MainView1")
...
function MainView:ctor(delegate)
    ...
    self.cityScrollView:jumpToBottom()
...
function MainView:showMainBg(delegate)
    ...
    if self.model.userInfoEntity.level >= 30 then
...
function MainView:__updateCityPosByGuide()
    ...
    if qy.isNoviceGuide then
        --训练场
        -- self.cityScrollView:jumpToRight()
        self.strongBtn:setVisible(false)
        self.help_btn:setVisible(false)
        self.cityScrollView:scrollToPercentBothDirection(cc.p(55,300), 0.0, false);
    end
