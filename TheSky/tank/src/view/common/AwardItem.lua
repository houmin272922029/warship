...
function AwardItem.createAwardView(data, viewType, bgScale , showDetail , callback, expandArr)
    local itemData = AwardItem.getItemData(data,showDetail, bgScale,callback)
...
function AwardItem.getItemData(data,showDetail, bgScale, callback)
    local itemData = {}
    local entity = nil
    local awardType = qy.tank.view.type.AwardType
    ...
    elseif data.type == awardType.EXLIQUID then
        ...
        itemData.nameTextColor = qy.tank.utils.ColorMapUtil.qualityMapColor(5)
        itemData.quality = 5
    elseif data.type == awardType.POLISH_STONE then
        -- 洗练石
        itemData.bg = "Resources/common/item/item_bg_3.png"
        itemData.icon = qy.tank.utils.AwardUtils.getAwardIconByType(data.type)
        itemData.num = data.num
        itemData.name = "洗练石"
        itemData.intro = "装备洗练必备物品，可通过分解红色品质装备获得。"
        itemData.iconScale = 1
        itemData.nameTextColor = qy.tank.utils.ColorMapUtil.qualityMapColor(3)
        itemData.quality = 3
    elseif data.type == awardType.EXTRACTOR then
        -- 萃取器
        itemData.bg = "Resources/common/item/item_bg_4.png"
        itemData.icon = qy.tank.utils.AwardUtils.getAwardIconByType(data.type)
        itemData.num = data.num
        itemData.name = "萃取器"
        itemData.intro = "装备萃取必备物品，可通过分解红色品质装备获得。"
        itemData.iconScale = 1
        itemData.nameTextColor = qy.tank.utils.ColorMapUtil.qualityMapColor(4)
        itemData.quality = 4
    elseif data.type == awardType.BREAKTHROUGH_STONE then
        -- 突破石
        itemData.bg = "Resources/common/item/item_bg_6.png"
        itemData.icon = qy.tank.utils.AwardUtils.getAwardIconByType(data.type)
        itemData.num = data.num
        itemData.name = "突破石"
        itemData.intro = "装备突破必备物品，可使装备副属性达到极限。"
        itemData.iconScale = 1
        itemData.nameTextColor = qy.tank.utils.ColorMapUtil.qualityMapColor(6)
        itemData.quality = 6
    end
