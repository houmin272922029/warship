...
local PayRebateCell = qy.class("PayRebateCell", qy.tank.view.BaseView, "view/operatingActivities/pay_rebate/PayRebateCell")

local tankType = qy.tank.view.type.AwardType.TANK
local _moduleType = qy.tank.view.type.ModuleType.PAY_REBATE

function PayRebateCell:ctor(delegate)
    PayRebateCell.super.ctor(self)
...
function PayRebateCell:sendMsg(chooseIdx)
    ...
    service:getCommonGiftAward(id, _moduleType, true, function(reData)
        self.model:upRebateDataStaByIdx(self.idx)
        self:updateStatus(2)
    end)
end

function PayRebateCell:render(_idx)
    self.data = self.model:getPayRebateAwardByIndex(_idx)
    self.idx = _idx
    self.recharge_num:setString(qy.TextUtil:substitute(63014).. self.data.cash.. qy.TextUtil:substitute(63015))
    ...
    self.awardList:setPosition(190,240)
...
function PayRebateCell:createEquipSuitEffert(_target)
...
function PayRebateCell:updateStatus(_status)
	if _status == 0 then
