local Widget1 = qy.class("Widget1", qy.tank.view.BaseView)
...
function Widget1:ctor()
    Widget1.super.ctor(self)
    ...
    self:runAction(cc.Sequence:create(cc.DelayTime:create(0.1), cc.CallFunc:create(function()
        -- Model:connect()
        qy.Event.dispatch(Model.Event.MESSAGE, Model.data.world)
    end)))
end

function Widget1:makeItem(itemData)
...
function Widget1:handlerChannel(richText, channel)
    richText:pushBackElement(self:makeText("【" .. channel.name .. "】", channel.color))
end

function Widget1:handlerUser(richText, user)
    if not user then
        return
    end
...
function Widget1:handlerContent(richText, content)
...
function Widget1:makeText(text, color)
