--[[--
--充值 DailywelfareService
--Author: H.X.Sun
--Date: 2015-05-04
--]]

local DailywelfareService = qy.class("DailywelfareService", qy.tank.service.BaseService)
...
function DailywelfareService:paymentBegin(data, onSuccess,gift_type,gift_id)
    ...
    qy.Http.new(qy.Http.Request.new({
        ...
        print("DailywelfareService paymentBegin","channel="..qy.tank.utils.SDK:channel())
        --更新奖励显示仅针对每日福利
...
function DailywelfareService:paymentFinish(token,onSuccess,jdata)
    onSuccess(3)
    co(function()
        for i = 1, 3 do
            print("第" .. i .. "次查询")
            local status, msg = yield(DailywelfareService.check(token,i,jdata))
            if status then
                return true
            end
...
function DailywelfareService.check(token,delay,jdata)
