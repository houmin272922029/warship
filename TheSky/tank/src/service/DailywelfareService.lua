--[[--
--充值 daily_welfareService
--Author: H.X.Sun
--Date: 2015-05-04
--]]

local daily_welfareService = qy.class("daily_welfareService", qy.tank.service.BaseService)
...
function daily_welfareService:paymentBegin(data, onSuccess,gift_type,gift_id)
    ...
    qy.Http.new(qy.Http.Request.new({
        ...
        print("daily_welfareService paymentBegin","channel="..qy.tank.utils.SDK:channel())
        --更新奖励显示仅针对每日福利
...
function daily_welfareService:paymentFinish(token,onSuccess,jdata)
    onSuccess(3)
    co(function()
        for i = 1, 3 do
            print("第" .. i .. "次查询")
            local status, msg = yield(daily_welfareService.check(token,i,jdata))
            if status then
                return true
            end
...
function daily_welfareService.check(token,delay,jdata)
